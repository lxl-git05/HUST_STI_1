...
extern I2C_HandleTypeDef hi2c1; // HAL库硬件IIC确定IIC的总线(I2C1)
...
// 在 TIM16/TIM17 ISR 中也能正常工作（SysTick 无法抢占 ISR 但 DWT 是纯硬件计数器）
// H743 I2C v2 寄存器版：CR2/ISR/TXDR/RXDR/ICR
static HAL_StatusTypeDef MPU6050_I2C_BurstRead(uint8_t reg, uint8_t *buf, uint8_t size)
{
    ...
    // ----- 2. 清空上次通信残留标志（v2 标准做法，防止 NACK 等影响本次）-----
    i2c->ICR = 0x3F38UL;

    // ----- 3. 写相位：START + 从机地址(写) + 寄存器地址（NBYTES=1，无AUTOEND→发完置TC）-----
    i2c->CR2 = (uint32_t)MPU6050_ADDRESS
    ...
    // ----- 4. 读相位：重复START + 从机地址(读) + 读 size 字节 + AUTOEND(自动STOP) -----
    i2c->CR2 = (uint32_t)(MPU6050_ADDRESS | 0x01)
