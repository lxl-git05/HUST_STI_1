...
#else
/* ========================================================================
 * 硬件I2C实现（复用已配置的I2C1外设，注意与IMU共用总线时的冲突）
 * ======================================================================== */

/**
 * @brief AT24C02 初始化（硬件I2C1已在MX_I2C1_Init中初始化，此处为空操作）
 */
void AT24C02_Init(void)
{
    /* I2C1硬件已在main.c的MX_I2C1_Init()中初始化 */
}
...
uint8_t AT24C02_WriteByte(uint8_t WordAddress, uint8_t Data)
{
    ...
    status = HAL_I2C_Mem_Write(&hi2c1, AT24C02_ADDR_WRITE, WordAddress,
...
uint8_t AT24C02_ReadByte(uint8_t WordAddress)
{
    ...
    HAL_I2C_Mem_Read(&hi2c1, AT24C02_ADDR_READ, WordAddress,
                     I2C_MEMADD_SIZE_8BIT, &Data, 1, 100);
    return Data;
}
